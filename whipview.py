...
PLANNED_ID_RE = re.compile(r"^(?:task\s+)?([A-Za-z]*\d+[A-Za-z0-9]*)\b", re.IGNORECASE)
...
def file_birth(path: Path) -> float:
    """When the file was created. Falls back to mtime where birth time is unavailable."""
    stat = path.stat()
...
def clock_span(start: float | None, end: float | None) -> str:
    """Local start–end date and clock, e.g. 21.09.2026 14:59–15:14. Open runs omit the end."""
    if start is None:
        return ""

    def stamp(timestamp: float, with_date: bool) -> str:
        moment = datetime.fromtimestamp(timestamp)
        return moment.strftime("%d.%m.%Y %H:%M" if with_date else "%H:%M")

    if end is None:
        return f"{stamp(start, True)}–"
    different_day = datetime.fromtimestamp(start).date() != datetime.fromtimestamp(end).date()
    return f"{stamp(start, True)}–{stamp(end, different_day)}"
...
def task_state() -> list[dict]:
    ...
    for task_id, slug, prompt_path in prompt_files():
        ...
        run_start = file_birth(log_path) if log_path.is_file() else None
        run_end = report_path.stat().st_mtime if report_path.is_file() else None
        start_time = run_start if run_start is not None else prompt_stat.st_mtime
        end_time = run_end if run_end is not None else now
        ...
        tasks.append(
            {
                ...
                "clock": clock_span(run_start, run_end),
                "_prompt_mtime": prompt_stat.st_mtime,
...
def planned_task_id(item: str) -> str | None:
    match = PLANNED_ID_RE.match(item.strip())
    return match.group(1) if match else None


def planned_title(item: str, task_id: str) -> str:
    text = re.sub(r"(?i)^task\s+", "", item.strip())
    text = re.sub(rf"(?i)^{re.escape(task_id)}\s*[:—\-]*\s*", "", text).strip()
    return text or item.strip()


def duty_queue(tasks: list[dict], upcoming: object) -> list[dict]:
    """Queued duties in run order. `upcoming` is status.json `next` (first runs first).

    Entries already running, stalled, or done are skipped. Queued prompts that
    the plan does not mention follow, oldest prompt first.
    """
    planned = [item.strip() for item in upcoming if isinstance(item, str) and item.strip()] if isinstance(upcoming, list) else []
    queued = [task for task in tasks if task.get("status") == "queued"]
    by_id: dict[str, dict] = {}
    for task in queued:
        by_id.setdefault(str(task.get("id", "")).lower(), task)
    known = {str(task.get("id", "")).lower(): task for task in tasks}
    used: set[str] = set()
    ordered: list[dict] = []

    def add(task_id: str, title: str, has_prompt: bool) -> None:
        ordered.append({"id": task_id, "title": title, "prompt": has_prompt})

    for item in planned:
        task_id = planned_task_id(item)
        if task_id is None:
            add("", item, False)
            continue
        key = task_id.lower()
        ...
        if match is not None:
            used.add(key)
            add(str(match["id"]), str(match.get("title") or item), True)
            continue
        if key in known:
            continue
        used.add(key)
        add(task_id, planned_title(item, task_id), False)

    leftovers = [task for task in queued if str(task.get("id", "")).lower() not in used]
    leftovers.sort(key=lambda task: task.get("_prompt_mtime") or 0)
    for task in leftovers:
        key = str(task.get("id", "")).lower()
        if key in used:
            continue
        used.add(key)
        add(str(task["id"]), str(task.get("title") or ""), True)
    return ordered
...
const labels=['ID','Başlık','Durum','Model','Efor','Token','Süre','Saat'];
const stepCard=$('step').closest('.card');
stepCard.innerHTML='<div class="orch-head"><div><div class="eyebrow">Şu an</div><div id="step" class="current-step">—</div></div><div class="updated">Güncellendi <span id="updated">—</span></div></div><div id="notes"></div>';
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802 - required by BaseHTTPRequestHandler
        ...
        if path == "/":
            ...
            tasks = task_state()
            queue = duty_queue(tasks, orchestrator.get("next"))
            for task in tasks:
                task.pop("_prompt_mtime", None)
            self.send_json(
                {
